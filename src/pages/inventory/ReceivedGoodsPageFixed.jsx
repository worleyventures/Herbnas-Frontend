...
const ReceivedGoodsPageFixed = () => {
  ...
  // Load data function - NOT memoized
  const loadData = () => {
    if (loading || !isMountedRef.current) return;
    
    console.log('ReceivedGoodsPageFixed: Loading data...');
  ...
  useEffect(() => {
    console.log('ReceivedGoodsPageFixed: Component mounted');
  ...
  // Load data when filters change (with debouncing) - separate from loadData function
  useEffect(() => {
    if (!isMountedRef.current || !hasLoadedRef.current) return;
    
    console.log('ReceivedGoodsPageFixed: Filters changed, debouncing load...');
    ...
    timeoutRef.current = setTimeout(() => {
      if (isMountedRef.current && !loading) {
        ...
        console.log('ReceivedGoodsPageFixed: Debounced load with params:', params);
        dispatch(getAllSentGoods(params));
      }
  ...
  }, [dispatch, searchTerm, statusFilter, currentPage, loading]); // Direct dependencies

  // Cleanup on unmount
  useEffect(() => {
    return () => {
      console.log('ReceivedGoodsPageFixed: Component unmounting');
...
export default ReceivedGoodsPageFixed;
